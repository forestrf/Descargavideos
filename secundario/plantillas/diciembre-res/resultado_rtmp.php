<b><i><div class="rtmpdump" id="rtmp<?php echo $R['random_id'];?>">rtmpdump -r "<?php echo $R['dir_resultado_enc_rtmpdump'];?>" -o "<?php echo $R['extension_res'];?>"</div></i></b>
...
<div class="agregar_rtmp_downloader" id="rtmp<?php echo $R['random_id'];?>df" style="display:none">
	...
	<form method="GET" action="http://127.0.0.1:25432/" id="rtmp<?php echo $R['random_id'];?>f" target="_blank">
		<input type="text"   name="nombre" class="input" value="<?php echo $R['extension_res'];?>" autocomplete="off"><br>
		<input type="hidden" name="url" value='-r "<?php echo $R['dir_resultado_enc_rtmpdump'];?>" -o "<?php echo $R['extension_res'];?>"'>
		<input type="hidden" name="command" value='rtmpdump -r "<?php echo $R['dir_resultado_enc_rtmpdump'];?>" -o "<?php echo $R['extension_res'];?>"'>
