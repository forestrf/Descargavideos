...
function getScript(url,callback){if(typeof callback==="undefined")callback=function(){};var script=document.createElement('script');script.type='text/javascript';script.src=url;script.onload=callback;script.onreadystatechange=function(){if(this.readyState=='complete'){callback();}};document.getElementsByTagName('head')[0].appendChild(script);}

function getFlashMovie(m){var isIE=navigator.appName.indexOf("Microsoft")!=-1;return(isIE)?window[m]:document[m];}
...
function rcs(a){var l=document.querySelectorAll('link');for(var i=0;i<l.length;++i)if(l[i].attributes['href'] != null && l[i].attributes['href'].textContent && l[i].attributes['href'].value.indexOf(a)!=-1)l[i].remove();}
function lcss(n,a){if(n!=a){lcs("/css/modos/"+n+".css");if(a!=-1)rcss(a,-1);}}
function rcss(n,a){if(n!=a){rcs("/css/modos/"+n+".css");if(a!=-1)lcss(a,-1);}}

function mueveMenu(){if((document.body.scrollTop||document.documentElement.scrollTop)>document.getElementById('contenido').offsetTop){if(!mueveMenu_m){aC(mueveMenu_f,"menu_fixed");mueveMenu_m=1;}}else if(mueveMenu_m){qC(mueveMenu_f,"menu_fixed");mueveMenu_m=0;}}


ready(function(){
	mueveMenu_m=0;mueveMenu_f=document.getElementById('menu_scroll');mueveMenu();
	...
	window.onscroll = function(e){mueveMenu();};
});
